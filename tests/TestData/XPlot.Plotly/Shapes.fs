...
module Chart2 =
    ...
    let x0 = getNormalData 0
    let y0 = getNormalData 1

    let x1 = x0 |> Array.map (fun x -> x + 0.1)
    let y1 = y0 |> Array.map (fun y -> y + 0.1)

    let x2 = x1 |> Array.map (fun x -> x + 0.1)
    let y2 = y1 |> Array.map (fun y -> y + 0.1)
...
module Chart9 =
    let xValues = getLinearSpacedData 0
    let yValues =
        [|
            for x in 0 .. (xValues.Length - 1) ->
                xValues.[x] * sin (xValues.[x] ** 2.) + 1.0
        |]
